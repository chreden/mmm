...
namespace mmm
{
    ...
    std::shared_ptr<Entity> Entities::add(const std::string& odf, int team, const Vector3& position)
    {
    ...
    std::shared_ptr<Entity> Entities::add( const std::string& odf, const std::shared_ptr<Team>& team, const Vector3& position )
    {
        if(!team)
        {
            return {};
    ...
    void Entities::find(std::vector<std::shared_ptr<Entity>>& ents, FindComparator comparator, void* argument)
    {
        ...
        for(int i = 0, entsFound = 0; i < EntityPointerManager::MaxEntities; ++i)
        {
            ...
            {
                auto ent = createEntityPtr(entity.m_entity);
    ...
    void Entities::getActiveEntities(std::vector<std::shared_ptr<Entity>>& results)
    {
        const EntityPointerManager* const epm = GetEntityPointerManager();
        for (int i = 0; i < EntityPointerManager::MaxEntities; ++i)
        {
            const EntityPointer& entity = epm->m_entity_pointer[i];
            if (entity.m_id.m_used && !entity.m_entity->m_expired)
            {
    ...
    bool Entities::remove(const std::shared_ptr<Entity>& entity)
    {
        if (entity)
        {
