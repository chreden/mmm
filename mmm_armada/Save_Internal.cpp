...
namespace mmm
{
    namespace
    {
        ...
        int save_save(lua_State* L)
        {
            save.doSave();
            return 0;
        }

        int save_write(lua_State* L)
        {
            if (lua_type(L, 2) == LUA_TSTRING)
            {
                lua_pushboolean(L, save.write(lua_tostring(L, 3)));
                return 1;
            }
            else if (lua_type(L, 2) == LUA_TNUMBER)
            {
                lua_pushboolean(L, save.write(static_cast<float>(lua_tonumber(L, 3))));
                return 1;
            }
            else if (lua_type(L, 2) == LUA_TBOOLEAN)
            {
                lua_pushboolean(L, save.write(static_cast<bool>(lua_toboolean(L, 3))));
                return 1;
            }
            return 0;
        }

        int save_index(lua_State* L)
        {
            const std::string key = lua_tostring(L, 2);
            if (key == "save")
            {
                lua_pushcfunction(L, save_save);
                return 1;
            }
            else if (key == "write")
            {
                lua_pushcfunction(L, save_write);
                return 1;
            }
            else if (key == "position")
            {
                lua_pushnumber(L, save.getLength());
                return 1;
            }
            return 0;
        }

        int save_newindex(lua_State* L)
        {
            const std::string key = lua_tostring(L, 2);
            if (key == "position")
            {
                save.setWrite(lua_tonumber(L, 3));
                return 0;
            }
            return 0;
        }
    ...
    void Save::doSave()
    {
        // Remove any holders or mappers in the area.
        deleteSaveEntities();

        // Now that we know how the long the data is, we can write it to the packets (that we will be making).
        ...
        for(int i = 0; i < PacketsToMake; ++i)
        {
            ...
            auto holder = std::static_pointer_cast<Craft>(Entities::add(HolderName, 0, Vector3(1900 + static_cast<float>(i) * 50, 0, 2200)));
            auto mapper = std::static_pointer_cast<Craft>(Entities::add(MapperName, 0, Vector3(1900 + static_cast<float>(i) * 50, 0, 2150)));
    ...
    void Save::setWrite(int position)
    {
        position_ = position;
    }

    bool Save::write(const std::string& str)
    {
    ...
    void save_register(lua_State* L)
    {
        lua_newtable(L);
        create_metatable(L,
            {
                { "__index", save_index },
                { "__newindex", save_newindex },
            });
