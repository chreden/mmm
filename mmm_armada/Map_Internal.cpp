...
namespace mmm
{
    namespace
    {
        ...
        const std::size_t Offset_Offset = 96;
        ...
        int map_getvisibility(lua_State* L)
        {
            lua_pushnumber(L, map.getVisibility(get_userdata<Vector3>(L, 2), get_userdata<std::shared_ptr<Team>>(L, 3)));
            return 1;
        }

        int map_loadstarfield(lua_State* L)
        {
            map.loadStarfield(lua_tostring(L, 2), lua_tostring(L, 3));
            return 0;
        }

        int map_index(lua_State* L)
        {
            const std::string key = lua_tostring(L, 2);
            if (key == "getVisibility")
            {
                lua_pushcfunction(L, map_getvisibility);
                return 1;
            }
            else if (key == "loadStarfield")
            {
                lua_pushcfunction(L, map_loadstarfield);
                return 1;
            }
            else if (key == "min")
            {
                return vector_new(L, map.getMin());
            }
            else if (key == "max")
            {
                return vector_new(L, map.getMax());
            }
            return 0;
        }

        int map_newindex(lua_State* L)
        {
            const std::string key = lua_tostring(L, 2);
            if (key == "min")
            {
                map.setMin(get_userdata<Vector3>(L, 2));
                return 1;
            }
            else if (key == "max")
            {
                map.setMax(get_userdata<Vector3>(L, 2));
                return 1;
            }
            return 0;
        }
    ...
    void Map::updateMapSize()
    {
        ...
        // This gets rid of the problematic border which may be there if the map is growing to a size bigger than it was before.
        class QuadTree { };
    ...
    Map::eVisibility Map::getVisibility(const Vector3& position, const std::shared_ptr<Team>& player) const
    {
    ...
    {
        lua_newtable(L);
        create_enum(L, "Visibility",
            {
                { "Shrouded", Map::Shrouded },
                { "Fogged", Map::Fogged },
                { "Visible", Map::Visible }
            });
        create_metatable(L,
