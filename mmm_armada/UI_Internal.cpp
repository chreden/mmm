...
namespace mmm
{
	...
	int
	UI::addMessage(const std::string& text, int x, int y, mmm::eJustify justify, 
		const mmm::ST3D_Colour &colour, bool fade, float displayTime )
	{
	...
	int 
	UI::addTitle( const std::string& str, int x, int y, float time, bool fade )
	{
	...
	int 
	UI::addSubtitle( const std::string& str, int x, int y, float time, bool fade )
	{
	...
	void 
	UI::removeMessage( const UIMessage& message )
	{
		switch( message.type )
		{
		case MessageType_UI:
	...
	void 
	UI::fadeMessage( const UIMessage& message )
	{
		switch( message.type )
		{
		case MessageType_UI:
	...
	void
	UI::addMinimapMark( const Vector3& pos, int radius, float shrinkRate, const ST3D_Colour& color )
	{
	...
	void 
	UI::showTradesTo( TeamPtr player )
	{
